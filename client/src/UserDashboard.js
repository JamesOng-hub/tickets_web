...
function UserDashboard() {
  ...
  const sendDeleteReq = (ticketId) => {
    return fetch(`${process.env.REACT_APP_API_URL}/product/deleteTicket/${user._id}/${ticketId}`, {
      method: 'DELETE',
      headers: {
  ...
  return (
    <div className="pageBackground--bright">
      ...
      <div className="userDashboard__info-card">
        ...
        <div className="userDashboard__info-card-title-dets">{user.name}</div>
        <div className="userDashboard__info-card-title-dets">{user.email}</div>
        {/* <div>
          UserType:{" "}
          ...
          <div key={ticket._id} className="userDashboard__ticket-card">
            <div className="d-flex flex-column justify-content-between">
              <div>
                ...
                <div className="userDashboard__ticket-card-desp">{ticket.description}</div>
              </div>
              <div className="userDashboard__ticket-card-dets">
                <div className="mx-1"><SellIcon/> RM{ticket.price}</div>
                <div className="mx-1"><DateRangeIcon/> {ticket.date}, {ticket.time}</div>
                {/* <div><AccessTimeIcon/> {ticket.time}</div> */}
                <div className="mx-1"><LocationOnIcon/> LOCATION</div>
              </div>
            </div>
            <div>
              <Link to={`/updateTicket/${ticket._id}`}>
                <button variant="contained" color="primary" className=" userDashboard__ticket-card-btn btn btn-outline-dark">
                  Edit Details
                </button>
                <button variant="contained" color="primary" className=" userDashboard__icon-btn btn btn-outline-primary">
                  <EditIcon/>
                </button>
              </Link>
              <div>
                <button className="userDashboard__ticket-card-btn btn btn-outline-danger" onClick={() => deleteTicket(ticket._id)}>
                <DeleteOutlineIcon/> Delete
                </button>
                <button className="userDashboard__icon-btn btn btn-outline-danger" onClick={() => deleteTicket(ticket._id)}>
                <DeleteOutlineIcon/> 
                </button>
              </div>
            </div>
